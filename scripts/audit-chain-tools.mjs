...
async function auditInteractions(browser){
  ...
  try{
    ...
    const trigger=page.locator(`.tools-row[data-tool-id="${first.toolId}"] [data-hover-tool]`);await trigger.hover();await page.waitForTimeout(220);
    let hover=await page.evaluate(({toolId,url})=>{const card=document.querySelector('.tools-hoverdoc:not([hidden])'),source=card?.querySelector('a[target="_blank"]');return {count:document.querySelectorAll('#toolsHoverdoc').length,visible:!!card,described:document.querySelector(`[data-hover-tool="${CSS.escape(toolId)}"]`)?.getAttribute('aria-describedby'),definition:card?.querySelector('p')?.textContent||'',details:!!card?.querySelector('[data-hover-details]'),href:source?.href,rel:source?.rel,text:source?.textContent}}, {toolId:first.toolId,url:toolById.get(first.toolId).officialUrl});
    check(hover.count===1&&hover.visible&&hover.described==='toolsHoverdoc'&&hover.definition===toolById.get(first.toolId).summary&&hover.details&&hover.href===new URL(toolById.get(first.toolId).officialUrl).href&&/noopener/.test(hover.rel)&&/↗/.test(hover.text),`runtime: pointer Hoverdoc contract differs ${JSON.stringify(hover)}`);
