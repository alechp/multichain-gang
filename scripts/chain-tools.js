(function chainTools(scope){
  ...
  const ready=!!(scope?.Overlay&&scope?.Router&&scope?.Store&&scope?.positionOverlay&&shell&&page);
  ...
  let activeView='',activeSlug='',routeTrigger=null,closingForRoute=false,filterState=null,expandedTool='',compareIds=[],hoverTimer=0,hoverdoc=null,hoverTrigger=null,hoverPinned=false,heldControl=false,hoveredRow=null,searchTimer=0,lastRenderedHash='';
  ...
  function present(focusTarget){
    ...
    if(!scope.Overlay.isOpen(shell))scope.Overlay.open({element:shell,trigger:routeTrigger,focusTarget:focusTarget||page.querySelector('h1'),modal:true,scrim:false,lockScroll:true,outsideClose:false,sheet:false,onClose:()=>{closeHoverdoc();clearControl();activeView='';activeSlug='';document.title=baseTitle;if(closingForRoute)closingForRoute=false;else scope.Router.close()}});
  ...
  function renderDirectory(){
    ...
    page.append(main);bindDirectorySearch(searchInput,searchResults);applyVeil();applyTerms();return true;
  }
  ...
  function renderChain(slug){
    ...
    const filterDisclosure=node('details',{class:'tools-filter-disclosure',open:true},[text('summary','','FILTERS')]);const filters=node('div',{class:'tools-filters'});
    ...
    const tableMount=node('div',{class:'tools-table-mount'});const live=text('p','tools-live','');live.id='toolsLive';live.setAttribute('role','status');live.setAttribute('aria-live','polite');const compareBar=node('div',{class:'tools-compare-bar',hidden:true},[text('p','', ''),node('button',{type:'button',class:'tools-compare-open',text:'OPEN COMPARISON'})]);compareBar.querySelector('button').addEventListener('click',openCompare);inventory.append(inventoryHead,searchLine,filterDisclosure,tableMount,compareBar,live);main.append(inventory);
    ...
    page.append(main);bindChainControls(chain,{coverage,search:searchLabel.querySelector('input'),tableMount,resultCount,compareBar,live});renderInventory();applyVeil();applyTerms();requestAnimationFrame(()=>restoreDeepLink());return true;
  }
  ...
  function renderInventory(focusAfter){
    ...
    updateCompareBar(bar);if(focusAfter instanceof HTMLElement&&focusAfter.isConnected)focusAfter.focus({preventScroll:true});else if(previousFocus?.closest?.('.tools-row')&&!previousFocus.isConnected)page.querySelector('#toolsQuery')?.focus({preventScroll:true});bindTableEvents(mount);if(expandedTool)requestAnimationFrame(()=>restoreDeepLink(false));
  }
  ...
  function applyVeil(){if(!shell)return;const enabled=veilEnabled();shell.dataset.linkVeil=enabled?'true':'false';if(veilButton){veilButton.setAttribute('aria-pressed',enabled?'true':'false');const label=veilButton.querySelector('span');if(label)label.textContent=enabled?'LINK VEIL · HOLD CTRL':'LINKS VISIBLE';else veilButton.textContent=enabled?'LINK VEIL · HOLD CTRL':'LINKS VISIBLE'}clearControl()}
  function toggleVeil(){const enabled=!veilEnabled();veilStore?.set?.({v:1,enabled});applyVeil();const live=page?.querySelector('#toolsLive');if(live)live.textContent=enabled?'Link Veil enabled. Hover a row and hold Control to reveal its details route.':'Links visible.'}
  function editable(target){return !!target?.closest?.('input,textarea,select,[contenteditable="true"],[role="combobox"]')}
  function updateControl(){page?.querySelectorAll('.is-control-peek').forEach(row=>row.classList.remove('is-control-peek'));if(heldControl&&hoveredRow&&veilEnabled())hoveredRow.classList.add('is-control-peek')}
  function clearControl(){heldControl=false;page?.querySelectorAll('.is-control-peek').forEach(row=>row.classList.remove('is-control-peek'))}
  ...
  closeButton?.addEventListener('click',()=>scope.Overlay.close(shell,{reason:'button'}));veilButton?.addEventListener('click',toggleVeil);
  ...
  addEventListener('keydown',event=>{if(event.key==='Control'&&!event.repeat&&!editable(event.target)&&scope.Overlay.isOpen(shell)&&veilEnabled()&&!event.metaKey&&!event.altKey&&!event.shiftKey){heldControl=true;updateControl();return}if(event.key==='Escape'&&hoverdoc&&scope.Overlay.top()?.element===shell){event.preventDefault();event.stopPropagation();closeHoverdoc();return}if(event.key==='/'&&activeView==='directory'&&!editable(event.target)&&!event.metaKey&&!event.ctrlKey&&!event.altKey&&scope.Overlay.top()?.element===shell){event.preventDefault();page.querySelector('#toolsGlobalQuery')?.focus()}},{capture:true});
  addEventListener('keyup',event=>{if(event.key==='Control')clearControl()},{capture:true});addEventListener('blur',()=>{clearControl();closeHoverdoc()});document.addEventListener('visibilitychange',()=>{if(document.hidden){clearControl();closeHoverdoc()}});document.addEventListener('pointerdown',event=>{if(hoverPinned&&hoverdoc&&!hoverdoc.contains(event.target)&&event.target!==hoverTrigger)closeHoverdoc()},{capture:true});
  applyVeil();
