(function chainTools(scope){
  ...
  const siteAsset=value=>value?`${window.__multichainRoot||''}${String(value).replace(/^\.\//,'')}`:'';
  ...
  function normalizeChain(value,slug,categories,input){
    ...
    chain.image={small:siteAsset(image.small||image.src960||`assets/chain-tools/${chain.slug}-landscape-960.webp`),large:siteAsset(image.large||image.src1440||`assets/chain-tools/${chain.slug}-landscape-1440.webp`),master:siteAsset(image.src||''),caption:image.caption||raw.visualCaption||'Illustrative system structure. The verified inventory below is authoritative.',position:image.position||image.objectPosition||'50% 50%'};
    return chain;
  }
  ...
  function renderDirectory(){
    if(!ready)return false;
    activeView='directory';activeSlug='';lastRenderedHash=location.hash;clearPage();setAccent(null);shell.dataset.view='directory';if(ref)ref.textContent=`CHAIN TOOLS · ${catalog.chains.length} LANDSCAPES · ${catalog.tools.length} CANONICAL TOOLS`;if(prev)prev.hidden=true;if(next)next.hidden=true;document.title='Chain Tools — Multichain Gang';
  ...
  function renderChain(slug){
    ...
    activeView='chain';activeSlug=slug;lastRenderedHash=location.hash;clearPage();setAccent(chain);shell.dataset.view='chain';filterState=readFilters();if(ref)ref.textContent=`${chain.short} · TOOL LANDSCAPE · VERIFIED ${chain.verified}`;document.title=`${chain.name} tools — Multichain Gang`;
