(function chainTools(scope){
  ...
  let activeView='',activeSlug='',routeTrigger=null,closingForRoute=false,filterState=null,expandedTool='',compareIds=[],hoverTimer=0,hoverdoc=null,hoverTrigger=null,hoverPinned=false,heldControl=false,hoveredRow=null,hoveredToolTrigger=null,searchTimer=0,lastRenderedHash='';
  ...
  function toggleVeil(){const enabled=!veilEnabled();veilStore?.set?.({v:1,enabled});applyVeil();const live=page?.querySelector('#toolsLive');if(live)live.textContent=enabled?'Link Veil enabled. Hold Control while hovering to reveal details and definition previews.':'Links and definition previews visible.'}
  ...
  function clearControl(){heldControl=false;page?.querySelectorAll('.is-control-peek').forEach(row=>row.classList.remove('is-control-peek'));if(veilEnabled()&&!hoverPinned)closeHoverdoc()}
  ...
  addEventListener('keydown',event=>{if(event.key==='Control'&&!event.repeat&&!editable(event.target)&&scope.Overlay.isOpen(shell)&&veilEnabled()&&!event.metaKey&&!event.altKey&&!event.shiftKey){heldControl=true;updateControl();if(hoveredToolTrigger?.dataset.hoverTool){clearTimeout(hoverTimer);hoverTimer=setTimeout(()=>showHoverdoc(hoveredToolTrigger.dataset.hoverTool,hoveredToolTrigger),120)}return}if(event.key==='Escape'&&hoverdoc&&scope.Overlay.top()?.element===shell){event.preventDefault();event.stopPropagation();closeHoverdoc();return}if(event.key==='/'&&activeView==='directory'&&!editable(event.target)&&!event.metaKey&&!event.ctrlKey&&!event.altKey&&scope.Overlay.top()?.element===shell){event.preventDefault();page.querySelector('#toolsGlobalQuery')?.focus()}},{capture:true});
