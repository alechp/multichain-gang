...
for(const needle of ['scope.LinkVeil=LinkVeil','scope.GlobalChrome=api','ResizeObserver','--scope-global-h','scope.linkVeil','chain.linkVeil','canPointerPreview','registerControl'])check(chromeSource.includes(needle),`static: global controller omits ${needle}`);
...
async function auditWidth(browser,width){
  ...
  try{
    ...
    const root=await page.evaluate(()=>{
      ...
      return {header:header.getBoundingClientRect(),chapters:chapters.length,chaptersVisible:chapters.every(node=>node.getClientRects().length),controls:controls.map(node=>node.id),globalDisabled:document.getElementById('globalVeil')?.disabled,overflow:document.documentElement.scrollWidth-document.documentElement.clientWidth};
    });
    ...
    for(const item of routeCases){
      ...
      const result=await page.evaluate(item=>{
        ...
        return {hash:location.hash,persistent:header.dataset.persistenceProbe,header:hb,shell:sb,rail:rb,heading:mb,controls:controls.map(node=>({id:node.id,disabled:node.disabled,tabIndex:node.tabIndex})),modal:shell.getAttribute('aria-modal'),overflow:document.documentElement.scrollWidth-document.documentElement.clientWidth,chapters:Array.from(document.querySelectorAll('.chapter-subnav a')).filter(node=>node.getClientRects().length).length};
      },item);
...
async function auditInteractions(browser){
  ...
  try{
    await page.evaluate(()=>SCOPE.LinkVeil.set(false,{announce:false}));
